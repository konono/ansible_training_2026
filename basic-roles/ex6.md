...
### Roleは「約束事（コンベンション）」で成り立つ世界

roleの仕組みを理解する上で最も重要なことは、**roleは設定ファイルで定義するものではなく、ディレクトリ構造とファイル名の「約束事」によって成り立っている**ということです。

Ansibleはroleを読み込む際、決められた名前のディレクトリの中にある決められた名前のファイルを自動的に探索します。つまり、「このディレクトリにこの名前のファイルを置けば、Ansibleがそれを認識してくれる」という暗黙のルールに従ってroleは構成されています。
...
| ディレクトリ | 役割 |
|---|---|
| `tasks/` | roleが実行するタスクを格納する |
| `handlers/` | タスクから `notify` で呼び出されるハンドラを格納する |
| `defaults/` | 外部から上書き可能なデフォルト変数を格納する |
| `vars/` | role内部で使用する固定変数を格納する |
| `templates/` | Jinja2テンプレートファイルを格納する |
| `files/` | `copy` モジュール等で配布する静的ファイルを格納する |
| `meta/` | roleの依存関係などのメタ情報を格納する |

**ファイル名の約束:**

- `tasks/`、`handlers/`、`defaults/`、`vars/`、`meta/` の各ディレクトリでは、**エントリーポイントとなるファイル名は必ず `main.yml`** でなければなりません。Ansibleはこれらのディレクトリ内の `main.yml` だけを自動的に読み込みます。
- `templates/` と `files/` には任意の名前のファイルを配置できます（タスクから明示的にファイル名を指定して参照するため）。

**不要なディレクトリは省略できる:**

- 全てのディレクトリを必ず用意する必要はありません。roleに必要なディレクトリだけを作成すれば十分です。例えば、ハンドラを使わないroleであれば `handlers/` ディレクトリは不要です。
- Ansibleは存在するディレクトリだけを読み込み、存在しないディレクトリについてはエラーにはなりません。

この「約束事ベース」のアプローチにより、roleの構造を見ただけでどこに何が定義されているかが予測でき、roleを書く人も読む人も共通のルールに従ってコードを理解できるようになっています。
...
上記のディレクトリツリーこそが、先ほど説明した「約束事」の具体的な姿です。各ディレクトリ名（`tasks/`、`handlers/` など）や、その中に置かれる `main.yml` というファイル名はAnsibleが定めた規約であり、この命名規則に従うことでAnsibleが自動的にroleの構成要素を認識・読み込みしてくれます。設定ファイルで「タスクはこのファイルです」と指定する必要はありません。
